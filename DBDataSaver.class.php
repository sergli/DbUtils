...
require_once __DIR__ . '/DBDataSaverException.class.php';
...
abstract class DBDataSaver implements iDBDataSaver {
	...
	private function _cleanRow(array $row) {
		...
		foreach ($this->_columns as $field => $dataType) {
			if (!array_key_exists($field, $row)) {
				throw new DBDataSaverException(
	...
	protected function _setColumns(array $columns) {
		if (empty($columns)) {
			throw new DBDataSaverException("Пустой массив полей",
				self::E_NO_COLUMNS);
		}
	 ...
	 * @param mixed $db экземпляр соединения с БД. если не указано 
	 *	- получается из RDBTable::getConnection()
	 * return void
	 * @see getColumns()
	 * @see getConnection()
	 */
	public function __construct(RDBTable $table,  
		array $columns = null, $db = null) {
	...
	public function setBatchSize($size) {
		...
		throw new DBDataSaverException("Неверное значение для параметра",
